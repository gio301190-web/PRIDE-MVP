  ...
  app.innerHTML = `
    ...
    <main class="page">
      ...
      <section class="card" style="max-width:560px;margin:0 auto;">
        ...
        <div class="field">
          <label>ИМЯ И ФАМИЛИЯ</label>
          <input id="np_name" type="text" placeholder="Имя Фамилия">
        </div>
...
async function submitRegisterPlayer() {
  ...
  if (!name || !phone || !email || !walletId || !pass || !pass2) {
    return alert("Заполните имя, телефон, email, номер Wallet и пароль.");
  }
  ...
  app.innerHTML = `
    ...
    <main class="page">
      ...
      <section class="hero" style="text-align:left;">
        <h1 style="font-size:clamp(28px,4.2vw,52px);letter-spacing:.01em;white-space:normal;">
          ИГРАЙ, ПРОДАВАЙ, ЗАРАБАТЫВАЙ, ОБУЧАЙСЯ
        </h1>
      </section>
      ...
      <div class="wallet-layout">
        ...
        <section class="wallet-header" style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:16px;">
          ...
          <div class="prd-sale-widget">ВНУТРИИГРОВАЯ ЭКОСИСТЕМА<br><strong>THE GAME</strong></div>
        </section>
        ...
        <section class="wallet-finance-center">
          <div class="wallet-finance">
            ...
            <div class="wallet-finance-row">
              <span>ЛИЧНЫЕ ЕДИНИЦЫ</span>
              <strong>${esc(currentPlayer?.personal_units ?? 0)}</strong>
            </div>

            <div class="wallet-finance-row">
              <span>КОМАНДНЫЕ ЕДИНИЦЫ</span>
              <strong>${esc(currentPlayer?.team_units ?? 0)}</strong>
            </div>

            <div class="wallet-finance-row">
              <span>ОБЩИЕ ЕДИНИЦЫ</span>
