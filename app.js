...
async function submitRepayLoan() {
  if (!confirm("Закрыть займ? Сумма займа спишется с баланса, залог вернётся в доступ.")) return;
  ...
  try {
    ...
    alert(`Займ закрыт. Списано: ${data.repaid} PRD. Возвращено из залога: ${data.released} PRD.`);
    walletLoan();
  } catch (error) {
    showError(error);
  }
