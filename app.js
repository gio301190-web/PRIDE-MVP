...
async function confirmEnroll2FA() {
  ...
  try {
    ...
    walletSecurity(securityReturnTo);
  } catch (error) {
    showError(error);
  }
}
...
let securityReturnTo = "wallet";

async function walletSecurity(returnTo = "wallet") {
  securityReturnTo = returnTo;
  if (!currentUser || !currentProfile) return auth("login");
  ...
  app.innerHTML = `
    ...
    ${backBar(returnTo === "player" ? "player()" : "wallet()")}
  `;
}
...
async function monthlyPromo() {
  if (!currentUser || (currentProfile?.role !== "SELLER" && currentProfile?.role !== "FOUNDER")) {
    return alert("Промо доступно только Player.");
  }
  ...
  const { data: historyRows } = await supabaseClient
    ...
    .eq("wallet_id", currentPlayer?.salary_wallet_id)
    .eq("reference_type", "MONTHLY_PROMO")
    .order("created_at", { ascending: false });
  ...
  const monthlyPersonal = Number(currentPlayer?.monthly_personal_units || 0);
  ...
  app.innerHTML = `
    ...
    <main class="page">
      ...
      <section class="hero">
        <h1>Ежемесячное промо</h1>
        <p class="muted">
          Личные единицы за текущий календарный месяц: <strong>${esc(monthlyPersonal)}</strong>
        </p>
      </section>
      ...
      <section class="card" style="max-width:560px;margin:0 auto 24px;">
        ...
        <p class="muted" style="line-height:1.6;">
          За личные единицы, набранные за календарный месяц, начисляется бонус:<br>
          • 300 личных единиц — <strong>300 PRD</strong><br>
          • 500 личных единиц — <strong>500 PRD</strong><br>
          • 1000 личных единиц — <strong>1000 PRD</strong><br><br>
          Бонусы суммируются: например, 1800 единиц за месяц = 1000 + 500 + 300 = 1800 PRD.
          Бонус начисляется автоматически 1 числа следующего месяца.
        </p>
      </section>

      <section class="card">
        <div class="eyebrow">ИСТОРИЯ НАЧИСЛЕНИЙ</div>
        ${
          historyHtml
            ? `<div class="table-wrap">
                <table>
            ...
            : `<div class="empty">Бонусов пока не было.</div>`
        }
      </section>
...
function checkPromoBonus() {
  if (!currentPlayer) return;
  if (document.getElementById("promo-bonus-overlay")) return;

  if (currentPlayer.last_promo_notified === false && Number(currentPlayer.last_promo_bonus_amount) > 0) {
    showPromoBonusModal(currentPlayer.last_promo_bonus_amount, currentPlayer.last_promo_bonus_period);
  }
}

function showPromoBonusModal(amount, period) {
  const money =
    new Intl.NumberFormat("ru-RU", { maximumFractionDigits: 2 }).format(amount || 0) + " PRD";

  const overlay = document.createElement("div");
  overlay.id = "promo-bonus-overlay";
  overlay.style.cssText =
    "position:fixed;inset:0;background:rgba(0,0,0,.8);display:flex;align-items:center;justify-content:center;z-index:9999;padding:20px;";
  ...
  overlay.innerHTML = `
    <div style="background:linear-gradient(145deg,#151821,#0e1016);border:1px solid var(--accent);border-radius:22px;padding:40px 28px;text-align:center;max-width:380px;box-shadow:0 30px 90px #000a;">
      <div style="font-size:52px;margin-bottom:6px;">🎉💰🎉</div>
      <div class="eyebrow">ЕЖЕМЕСЯЧНОЕ ПРОМО</div>
      <h1 style="font-size:28px;margin:12px 0;">ПОЗДРАВЛЯЕМ!</h1>
      <p style="font-size:17px;font-weight:800;margin-bottom:8px;line-height:1.4;">
        Вы выполнили условия ежемесячного промо за ${esc(period || "прошлый месяц")}
      </p>
      <p style="font-size:22px;font-weight:900;color:#4fce6a;margin-bottom:24px;">
        +${money}
      </p>
      <button class="btn primary" style="width:100%;" onclick="closePromoBonusModal()">
        СПАСИБО
      </button>
    </div>
  `;
...
async function closePromoBonusModal() {
  const overlay = document.getElementById("promo-bonus-overlay");
  if (overlay) overlay.remove();

  try {
    await supabaseClient.rpc("acknowledge_promo_bonus");
    if (currentPlayer) {
      currentPlayer.last_promo_notified = true;
    }
  } catch (error) {
    console.error(error);
  }
      ...
      <div class="wallet-layout">
        ...
        <section class="wallet-sidebar">
          <div class="wallet-menu">
            ...
            <button class="btn" onclick="monthlyPromo()">ЕЖЕМЕСЯЧНОЕ ПРОМО</button>
            <button class="btn" onclick="walletSecurity('player')">БЕЗОПАСНОСТЬ</button>
          </div>
        </section>
