  ...
  app.innerHTML = `
    ...
    <main class="page">
      <section class="hero">
        ...
        <p class="muted">
          Найдите своего клиента по ID и сделайте его Player.
  ...
  try {
    ...
    resultBox.innerHTML = `
      <div class="card" style="background:var(--panel2);">
        ...
        <p class="muted" style="margin-top:6px;">ID: ${esc(profile.pride_id)}</p>
  ...
  if (isFounderWallet) {
    ...
    app.innerHTML = `
      ...
      <main class="page">
        ...
        <section class="hero">
          <h1>Founder</h1>
          <p style="font-size:16px;font-weight:800;color:var(--muted);margin-top:8px;">${esc(walletId)}</p>
        </section>

        <section class="grid">
          <div class="card"><div class="card-label">ОСТАТОК В РЕЗЕРВЕ</div><div class="big-value">${new Intl.NumberFormat("ru-RU").format(currentWallet.free_balance)}</div></div>
          <div class="card"><div class="card-label">ПРОДАНО PRD</div><div class="big-value">${new Intl.NumberFormat("ru-RU").format(sold)}</div></div>
          <div class="card"><div class="card-label">ТЕКУЩАЯ ЦЕНА</div><div class="big-value">$${(0.01 + Math.floor(sold / 100000) * 0.01).toFixed(2)}</div></div>
        </section>

        <section class="card">
          <div class="eyebrow">ВНУТРЕННИЙ ПЕРЕВОД</div>
          <p class="muted" style="margin-bottom:14px;">Перевести PRD с этого резерва на другой Wallet.</p>
          <button class="btn primary" style="width:100%;" onclick="walletTransfer()">СДЕЛАТЬ ПЕРЕВОД</button>
        </section>
        ...
        <section class="card" style="max-width:420px;margin:0 auto;">
          ...
          <button class="btn primary" style="width:100%;" onclick="changePassword()">СОХРАНИТЬ ПАРОЛЬ</button>
        </section>
      ...
      <main class="page">
        ...
        <section class="grid">
          <div class="card">
            ...
            <div class="big-value" style="color:${technikBalance < 0 ? "#e0554f" : "var(--text)"};">
              ${technikBalance < 0 ? "−" : ""}${moneyPrd(Math.abs(technikBalance))}
            </div>
          </div>
          <div class="card">
            <div class="card-label">ПОЛУЧЕНО КОМИССИОННЫХ</div>
            <div class="big-value" style="color:#4fce6a;">+${moneyPrd(totalCommissions)}</div>
          </div>
          <div class="card">
            <div class="card-label">ВЫПЛАЧЕНО ПРИБЫЛИ</div>
            <div class="big-value" style="color:#e0554f;">−${moneyPrd(totalPaid)}</div>
          </div>
        </section>
